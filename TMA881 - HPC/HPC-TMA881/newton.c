  ...
  for (i = tx;  i < lines; i += numThreads ) {
    ...
    for (j = 0; j < lines; ++j){
      ...
      for (k = 0; k < 128; k++){
        ...
        break;}
        if (creal(z) > 1000000000. || creal(z) < -1000000000.){
        break;}
        if (cimag(z) > 1000000000. || cimag(z) < -1000000000.){
        break;}

	FunctionDerivate(&z,&derivate, d);
	functionValue = derivate * z - 1;

	if (creal(functionValue)*creal(functionValue) + cimag(functionValue)*cimag(functionValue) < 0.000001*d){
	  convergence[j] = k;
...
main(int argc, char *argv[])
{
  ...
  for ( int tx = 0; tx < numThreads; ++tx ) {
    float ib = 2.0 - (tx * stepSize);
    float ie = -2.0 + (numThreads-tx)*stepSize;
